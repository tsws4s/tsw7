...
/**   and (myEvent.endTime > ?2)
 */

@Entity
@NamedQueries({
		@NamedQuery(name = "findAllEvents", query = "select myEvent from Event myEvent"),
		@NamedQuery(name = "findAllEvents4tsw", query = "select myEvent from Event myEvent where tswAcctFk = ?1"),
		@NamedQuery(name = "findEventByDateAfter", query = "select myEvent from Event myEvent where (resourceFk = ?1)"),
		@NamedQuery(name = "findEventByAllDay", query = "select myEvent from Event myEvent where myEvent.allDay = ?1"),
		@NamedQuery(name = "findEventByGameId", query = "select myEvent from Event myEvent where gameFk = ?1"),
...
@Table(catalog = "tsws4s_4", name = "Event")
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(namespace = "tsw7/tsw/domain", name = "Event")
