...
public class SportpositionDAOImpl extends AbstractJpaDao<Sportposition>
	...
	public Sportposition findSportpositionByPrimaryKey(Integer sportPositionId, int startResult, int maxRows) throws DataAccessException {
		try {
			...
			return (tsw.domain.Sportposition) query.getSingleResult();
	...
	@SuppressWarnings("unchecked")
	@Transactional
	public Set<Sportposition> findAllSportpositionsForSport(int startResult, int maxRows, int sportId) throws DataAccessException {
		Query query = createNamedQuery("findAllSportpositionsForSport", startResult, maxRows, sportId);
		return new LinkedHashSet<Sportposition>(query.getResultList());
	}
